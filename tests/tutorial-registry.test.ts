...
describe("tutorial registry", () => {
  it("registers the URL Shortener, Rate Limiter, Pastebin, Notification Service, Distributed Cache, Ticket Booking, Video Streaming, Collaborative Doc Editor, Cloud Drive, Payment System, Distributed Logging, and Distributed Job Scheduler tutorials", () => {
    expect(Object.keys(tutorials).sort()).toEqual(["cloud-drive", "collaborative-doc-editor", "distributed-cache", "distributed-job-scheduler", "distributed-logging", "notification-service", "pastebin", "payment-system", "rate-limiter", "ticket-booking", "url-shortener", "video-streaming"]);
  });

  it("describes the URL Shortener's eighteen sections, the Rate Limiter's fifteen, the Pastebin's eighteen, the Notification Service's eighteen, the Distributed Cache's eighteen, the Ticket Booking's eighteen, the Video Streaming's eighteen, the Collaborative Doc Editor's eighteen, the Cloud Drive's eighteen, the Payment System's eighteen, the Distributed Logging's eighteen, and the Distributed Job Scheduler's eighteen", () => {
