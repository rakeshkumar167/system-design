...
describe("tutorial registry", () => {
  it("registers the URL Shortener, Rate Limiter, Pastebin, Notification Service, and Ticket Booking tutorials", () => {
    expect(Object.keys(tutorials).sort()).toEqual(["notification-service", "pastebin", "rate-limiter", "ticket-booking", "url-shortener"]);
  });

  it("describes the URL Shortener's eighteen sections, the Rate Limiter's fifteen, the Pastebin's eighteen, the Notification Service's eighteen, and the Ticket Booking's eighteen", () => {
