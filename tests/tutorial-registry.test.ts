...
describe("tutorial registry", () => {
  it("registers the URL Shortener, Rate Limiter, Pastebin, Notification Service, Distributed Cache, API Gateway, Ticket Booking, Video Streaming, Collaborative Doc Editor, Cloud Drive, Payment System, Distributed Logging, Distributed Job Scheduler, Maps and Navigation, Web Crawler, and Search Autocomplete tutorials", () => {
    expect(Object.keys(tutorials).sort()).toEqual(["api-gateway", "cloud-drive", "collaborative-doc-editor", "distributed-cache", "distributed-job-scheduler", "distributed-logging", "maps-navigation", "notification-service", "pastebin", "payment-system", "rate-limiter", "search-autocomplete", "ticket-booking", "url-shortener", "video-streaming", "web-crawler"]);
  });

  it("describes the URL Shortener's eighteen sections, the Rate Limiter's fifteen, the Pastebin's eighteen, the Notification Service's eighteen, the Distributed Cache's eighteen, the API Gateway's eighteen, the Ticket Booking's eighteen, the Video Streaming's eighteen, the Collaborative Doc Editor's eighteen, the Cloud Drive's eighteen, the Payment System's eighteen, the Distributed Logging's eighteen, the Distributed Job Scheduler's eighteen, the Maps and Navigation's eighteen, the Web Crawler's eighteen, and the Search Autocomplete's eighteen", () => {
  ...
  it("returns undefined for unregistered tutorials", () => {
    expect(getTutorial("news-feed")).toBeUndefined();
  });
});
