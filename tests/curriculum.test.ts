...
describe("curriculum", () => {
  ...
  it("exposes URL Shortener, Rate Limiter, Pastebin, Notification Service, Distributed Cache, API Gateway, Web Crawler, Search Autocomplete, Video Streaming, Ticket Booking, Payment System, Distributed Logging, Collaborative Doc Editor, Cloud Drive, Maps and Navigation, and Distributed Job Scheduler as available", () => {
    expect(
      problems.filter((p) => p.status === "available").map((p) => p.slug),
    ).toEqual(["url-shortener", "rate-limiter", "pastebin", "notification-service", "distributed-cache", "api-gateway", "web-crawler", "search-autocomplete", "video-streaming", "ticket-booking", "payment-system", "distributed-logging", "collaborative-doc-editor", "cloud-drive", "maps-navigation", "distributed-job-scheduler"]);
